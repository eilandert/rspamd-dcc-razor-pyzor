	...
	for {
		...
		if err != nil {
			if (isTimeout(err) || errors.Is(err, io.EOF)) && sb.Len() > 0 {
				return s, nil // idle/EOF: reply complete
			}
			return s, err
		}
		if !time.Now().Before(deadline) {
			if sb.Len() > 0 {
				return s, nil // deadline reached with data: treat as complete
			}
			return s, fmt.Errorf("response not received within %s", timeout)
		}
	}
}
